...
<body>
    ...
    <div class="main-content" id="mainContent">
        ...
        <section class="section-header text-sm md:text-xl">
            <h5 class="text-light">SUBJECT OFFERING PAGE FOR AY : <p id="shoolYear"></p>, SEMESTER</h5>
        </section>
        ...
        <div class="d-flex flex-row w-100 justify-content-between">
            ...
            <div class="form-group flex-grow-1 mx-2">
                ...
                <div class="d-flex align-items-center">
                    <input type="number" class="form-control me-2" id="progAcadstart" name="progAcadstart" placeholder="Start Year">
                    <span class="mx-2">-</span>
                    <input type="number" class="form-control" id="progAcadend" name="progAcadend" placeholder="End Year" readonly>
                </div>
            </div>
        </div>
        ...
        <section class="section-header text-sm mt-6">
            <h5 class="text-light">SUBJECT OFFERED BY <p id="collegeName"></p></h5>
        </section>
...
document.getElementById('progAcadstart').addEventListener('input', function () {
    ...
    if (!isNaN(startYear)) {
        if (startYear > currentYear) {
            this.value = currentYear; // Limit to current year
...
let start = $('#progAcadstart').val();
let end = $('#progAcadend').val();

console.log("dept: "+department);

$('#shoolYear').text(start+"-"+end);

$.ajax({
    url: 'programAPI.php',
    method: 'GET',
    data: {
        action: 'getFilteredPrograms2',
    ...
    success: function(response) {
        ...
        response.forEach(function(program) {
            let row = `<tr class="text-gray-700 bg-white">
                ...
                <td class="p-1 border text-center">${program.section}</td>
...
<body>
    <h5>SUBJECT OFFERED BY COLLEGE/DEPARTMENT</h5>
    <table class="min-w-full border border-gray-300 w-100">
        <thead>
            ${$("#program-table thead").html()}
        </thead>
        <tbody>
            ${content}
        </tbody>
...
success: function (response) {
    ...
    collegeSelect.off('change').on('change', function () {
        var selectedText = $(this).find('option:selected').text();
        $('#collegeName').html(selectedText !== 'Select College' ? selectedText : '');
    });
},
error: function (xhr, status, error) {
    console.log('Error Status:', status);
